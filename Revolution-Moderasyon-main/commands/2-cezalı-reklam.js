    ...
    async run(message, args, level) {
        ...
        if (!message.guild.members.cache.has(user.id)) {
            ...
            await cezalar.findOne({ user: user.id }, async (err, doc) => {
                ...
                let sonrakii = id + 1
                member.roles.set(member.roles.cache.has(this.client.config.roles.boosterRole) ? [this.client.config.roles.boosterRole, this.client.config.roles.adsRole] : [this.client.config.roles.adsRole]).catch(e => console.log(e))
                this.client.yolla("<@" + user + "> üyesine <@&852194278318407789> rolü verildi. (Ceza Numarası: `#" + sonrakii + "`)", message.author, message.channel)
                const başe = new Discord.MessageEmbed()
                    .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                    .setColor("RANDOM")
                    .setDescription(`${user} üyesine __${args.slice(1).join(" ")}__ sebebi ile <@&852194278318407789> rolü verildi.`)
                    .setFooter(`${moment(Date.parse(new Date().toLocaleString("tr-TR", { timeZone: "Asia/Istanbul" }))).format("LLL")}`)
                await this.client.channels.cache.get(this.client.config.channels.adsLogs).send(başe)
                if (!doc) {
                    const newPun = new cezalar({
                        user: user.id,
                        ceza: true,
